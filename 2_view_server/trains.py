...
def get_schedule():
    ...
    try:
        platform = departures['1'] + departures['2'] + departures['3']
        soonest_departure_123 = min([train for train in platform if train > time.time()])
    except ValueError:
        soonest_departure_123 = None

    try:
        platform = arrivals['1'] + arrivals['2'] + arrivals['3']
        soonest_arrival_123 = min([train for train in platform if train > time.time()])
    except ValueError:
        soonest_arrival_123 = None

    try:
        soonest_departure_s = min([train for train in departures['GS'] if train > time.time()])
    except ValueError:
        soonest_departure_s = None

    try:
        soonest_arrival_s = min([train for train in arrivals['GS'] if train > time.time()])
    except ValueError:
